...
::gpk::error_t					DX::D3DDeviceResources::CreateWindowSizeDependentResources() {
	...
	if (m_swapChain) { // If the swap chain already exists, resize it.
		...
		if (hr != DXGI_ERROR_DEVICE_REMOVED && hr != DXGI_ERROR_DEVICE_RESET) 
			...
			return HandleDeviceLost();	// If the device was removed for any reason, a new device and swap chain will need to be created. Everything is set up now. Do not continue execution of this method. HandleDeviceLost will reenter this method and correctly set up the new device.
		}
	} else {
	...
	gpk_hrcall(m_swapChain->GetBuffer(0, IID_PPV_ARGS(&dxgiBackBuffer)));
