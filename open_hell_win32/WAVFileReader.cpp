...
#pragma pack(pop)
    ...
    //---------------------------------------------------------------------------------
    HRESULT LoadAudioFromFile(::gpk::view_array<const wchar_t> szFileName, _Inout_ ::gpk::array_pod<uint8_t>& wavData) noexcept {
        // open the file
        DWORD bytesRead = 0;
    #if (_WIN32_WINNT >= _WIN32_WINNT_WIN8)
        ScopedHandle hFile(safe_handle(CreateFile2(szFileName.begin(), GENERIC_READ, FILE_SHARE_READ, OPEN_EXISTING, nullptr)));
    #else
...
//-------------------------------------------------------------------------------------
HRESULT DirectX::LoadWAVAudioFromFile(::gpk::view_array<const wchar_t> szFileName, ::gpk::array_pod<uint8_t>& wavData, const WAVEFORMATEX** wfx, ::gpk::view_array<const uint8_t> & audioBytes) noexcept {
...
HRESULT DirectX::LoadWAVAudioInMemoryEx(::gpk::view_array<const uint8_t> wavData, DirectX::WAVData& result) noexcept {
    ...
    bool dpds, seek;
    HRESULT hr = WaveFindFormatAndData(wavData, &result.wfx, result.AudioView, dpds, seek);
    if (FAILED(hr))
        return hr;

    hr = WaveFindLoopInfo(wavData, &result.loopStart, &result.loopLength);
    if (FAILED(hr))
        return hr;

    if (dpds)
    {
        hr = WaveFindTable(wavData, FOURCC_XWMA_DPDS, &result.seek, &result.seekCount);
        if (FAILED(hr))
            return hr;
    }
    else if (seek)
    {
        hr = WaveFindTable(wavData, FOURCC_XMA_SEEK, &result.seek, &result.seekCount);
        if (FAILED(hr))
            return hr;
    }
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_
HRESULT DirectX::LoadWAVAudioFromFileEx(::gpk::view_array<const wchar_t> szFileName, ::gpk::array_pod<uint8_t>& wavData, DirectX::WAVData& result) noexcept {
    DWORD bytesRead = LoadAudioFromFile(szFileName, wavData);
    if (FAILED(bytesRead))
        return bytesRead;
