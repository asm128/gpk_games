...
::gpk::error_t										draw										(::SApplication& app)											{	// --- This function will draw some coloured symbols in each cell of the ASCII screen.
	...
	{
		::the1::theOneDraw(app.TheOne, app.GUIRT, framework.FrameInfo.Seconds.Total, true);
		{
			...
			if(app.GUITexture2D) {
				...
				if(mappedTexture.pData) {
					for(uint32_t y = 0; y < app.GUIRT.metrics().y; ++y) {
						memcpy(&((char*)mappedTexture.pData)[y * mappedTexture.RowPitch], app.GUIRT.Color.View[y].begin(), app.GUIRT.Color.View[y].byte_count());
					}
				}
				...
				// noelia gomez 0800 123 3333
				{
					...
					::gpk::ptr_com<ID3D11Buffer>					vb						= app.GUIVertexBuffer;
					::gpk::ptr_com<ID3D11Buffer>					ib						= app.GUIIndexBuffer ;
					...
					context->VSSetConstantBuffers1	(0, 1, &app.GUIConstantBufferScene, nullptr, nullptr);	// Send the constant buffer to the graphics device.
					context->PSSetConstantBuffers1	(0, 1, &app.GUIConstantBufferScene, nullptr, nullptr);	// Send the constant buffer to the graphics device.
					context->PSSetConstantBuffers1	(1, 1, &app.GUIConstantBufferNode, nullptr, nullptr);	// Send the constant buffer to the graphics device.
